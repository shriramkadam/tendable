...
class LoginViewController: UIViewController {
    ...
    override func viewDidLoad() {
        ...
        self.btnGotoRegister.layer.cornerRadius = 25.0
    }
    ...
    @IBAction func loginUser() {
        ...
        if isValidEmail(emailID: email) == false {
            ...
            lblMessage.text = ""
            sendPostLoginRequest(userEmail: email, userPassword: password)
        }
    }
    ...
    func sendPostLoginRequest(userEmail: String, userPassword: String) {
        let parameters = ["email": userEmail, "password": userPassword]
        
        guard let url = URL(string: "\(UrlManager.baseUrlApi)\("login")") else { return }
        //print(url)
            ...
            if let httpResponse = response as? HTTPURLResponse {
                ...
                if httpResponse.statusCode == 200 {
                    DispatchQueue.main.async {
                        ...
                        self.lblMessage.text = "User logged in successfully"
                    }
                }
            }
